...
int nSpheres = 0;
vec3 center_world[7]; //In world coordinate
float radius[7];
...
//Transmision on/ off
bool useTrans = false;
//theta Refraction on/ off
bool useRefract = false;
// inner reflection on/off
bool useInReflect = false;
// reflection depth flag
bool isFirst = false;
...
float findIntersection(const Ray& ray, const vec3& center, float radius, vec3& p, vec3& n)
{
	...
	{
		n = normalize(center - p);	// Normal at the point
	}
...
{
	// Find the closest intersection within [ray.pG^ ray.pl]
	int iSphere = -1;
...
vec3
refraction(const vec3& n, const vec3& v)
{
	vec3 I_temp = normalize(v);
	vec3 N = n;
	float intensityCos = clamp(dot(n, I_temp), -1.0f, 1.0f);

	float etai = 2, etat = 3;


	if (intensityCos < 0) {
		intensityCos = -intensityCos;
	}
	else {
		swap(etai, etat);
	}
	...
	float k = 1 - eta * eta * (1 - intensityCos * intensityCos);
	return k < 0 ? vec3(0) : eta * v + (eta * intensityCos - sqrt(k)) * N;

}

vec3 intensity(const Ray& ray, const Light l[], int nLights, int depth, bool nonReflect,bool isinR, int E = -1)
{
	...
	if (iObject != -1)
	{
		...
		{
			if (nonReflect) break; // 표면반사 이외에는 phong 계산 무시
			//Shadow ray
			...
			if (jObject == -1) // Not shadowed
			{
				...
				//vec3 v_t = normalize(ray.p1 - ray.p0);
				vec3 r = normalize(reflect(l[i].p_eye, n));

				if (!nonReflect) I += phong(n, v, l[i], r);
			}
			else I += ambient(l[i]);
		...
		if (depth < DEPTH)
		{
			...
			if (useReflect)
			{
				...
				vec3 I_R = intensity(recursiveRay, l, nLights, depth + 1, false,false, iObject);
			...
			vec3 rr = (ray.p1 - ray.p0);
			vec3 refract_ray = refraction(n, rr); // 굴절 방향 계산
			vec3 p_move = p + 1.0E-2f * normalize(refract_ray); // 살짝 이격시켜서 교점으로 p가 다시 나오는걸 방지 
			vec3 prr = p + 1.0E10f * normalize(refract_ray);
			...
			if (useTrans)
			{
				...
				if (!useRefract) { // 굴절x 투과만 되는 경우
					vec3 n_prr = p + 1.0E10f * normalize(rr); // Point far away from p along rr
					vec3 n_p_move = p + 1.0E-2f * normalize(rr); // 살짝 이격시켜서 교점으로 p가 다시 나오는걸 방지 
					Ray n_transmisionRay(n_p_move, n_prr);

					I_T = intensity(n_transmisionRay, l, nLights, depth + 1, false,false, iObject); // 구 내부 벽을 그대로 지나가는 판정
				}
				else {// 내부 반사를 위해서 허공에서 쏜 판정(구 내부 허공)

					Ray transmisionRay(p_move, prr);

					
					if (!nonReflect)I_T = intensity(transmisionRay, l, nLights, depth + 1, true,false, -1); //내부에서 굴절된 광선
					else {
						if (isinR) 	I_T = intensity(transmisionRay, l, nLights, depth + 1, false, iObject); //내부 반사에서 밖으로
						else I_T = intensity(transmisionRay, l, nLights, depth , false, iObject); // 내부 굴절에서 밖으로
					}//밖으로 나가는 광선
				}
			...
			if (useRefract && useInReflect) // 내부반사 사용의 경우
			{
				vec3 I_Ri(0);

				if (nonReflect)
				{
					vec3 innerR = normalize(reflect(-rr, n));
					vec3 n_prr = p + 1.0E10f * innerR;
					vec3 n_p_move = p + 1.0E-2f * innerR;

					Ray n_transmisionRay(n_p_move, n_prr);
					I_Ri = intensity(n_transmisionRay, l, nLights, depth + 1, true,true, -1); // 내부에서 반사된 광선
				}

				for (int i = 0; i < 3; i++)
					I[i] += m_specular[i] * I_Ri[i];
			}
...
#pragma omp parallel for schedule(dynamic) if (useOpenMP) // static, guided, auto, runtime
	for (int i = 0; i < m; i++)
		for (int j = 0; j < n; j++)
		{
			...
			vec3	I = intensity(Ray(s, e), light, nLights, 1, false,false);
...
#pragma omp master
	...
	light[0].diffuse = 0.5f * vec3(1.0, 1.0, 1.0); light[0].specular = vec3(1.0, 1.0, 1.0);
	nLights++;
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods) {
	if (action == GLFW_PRESS || action == GLFW_REPEAT)
	{
		...
		case GLFW_KEY_A: useReflect = !useReflect; break;
		case GLFW_KEY_S: useRefract = !useRefract; break;
		case GLFW_KEY_D: useTrans = !useTrans; break;
		case GLFW_KEY_F:useInReflect = !useInReflect; break;
